...
func (m *HotkeyManager) Run(ctx context.Context) error {
	...
	for _, b := range m.bindings {
		// Best-effort: one failed chord (duplicate, OS-claimed) must not take
		// down every other hotkey - warn and keep going.
...
func keyFor(s string) (hotkey.Key, bool) {
	...
	case "printscreen", "prtsc", "prtscn", "snapshot":
		// PC keyboards on macOS deliver PrintScreen as F13, so the same chord
