...
echo "==> building cgo binaries (darwin/$HOST_ARCH): host + editor + settings"
mkdir -p "$DIST"
SETTINGS_BIN="$DIST/goshareit-settings"
LDFLAGS="-X github.com/Rake-Pro/GoShareIt/internal/core/version.Version=$VERSION"
CGO_ENABLED=1 GOOS=darwin GOARCH="$HOST_ARCH" go build -tags production -ldflags "$LDFLAGS" -o "$BIN" ./cmd/goshareit
CGO_ENABLED=1 GOOS=darwin GOARCH="$HOST_ARCH" go build -ldflags "$LDFLAGS" -o "$EDITOR_BIN" ./cmd/goshareit-editor
CGO_ENABLED=1 GOOS=darwin GOARCH="$HOST_ARCH" go build -tags production -ldflags "$LDFLAGS" -o "$SETTINGS_BIN" ./cmd/goshareit-settings
